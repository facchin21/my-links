  ...
  return (
    <>
      <a
        ...
        aria-label='button social media'
        href={link}
        className={`w-8/12 max-w-container py-3 flex items-center justify-between
          font-poppins ${darkMode ? 'bg-bgDark text-textDark' : 'bg-bgLight text-textLight'} rounded-xl px-8 
          cursor-pointer transition-transform duration-300 
          hover:scale-105 shadow-md`}
      >
        {
          typeof Icon === 'string' ? (
            <img src={Icon} alt={`Image de ${name}`} className='w-9 h-9' />
          ) : (
            <Icon size={32} />
          )
        }
